...
filtered_data = [entry for entry in data if entry['label'] != 'petting' or
                 (entry['label'] == 'petting' and entry['sep'] == True) or
                 (entry['label'] == 'petting' and np.any(entry['frame'] != empty))]

# Save the filtered data in a new json file
with open('json_files/gesturespersons1-20WOhitting_pokpetEmptiesRemoved..json', 'w') as json_file:
